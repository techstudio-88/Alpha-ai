...
export async function POST(request){
 try{
  ...
  const job=await supabase.from("processing_jobs").insert({workspace_id:workspaceId,project_id:project.data.id,job_type:"ingest",status:"queued",progress:0,payload:{source_type:"google_photos",source_id:source.data.id,google_photos_base_url:sourceUrl,google_photos_media_id:externalId,file_name:mediaFile.filename||null}}).select().single();if(job.error)throw job.error;
