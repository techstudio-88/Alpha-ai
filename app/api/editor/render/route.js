...
export async function POST(request){
  try{
    ...
    const{workspaceId,projectId,mediaAssetId,startSeconds,endSeconds,title,aspect="9:16",speed=1,zoom=1,aiPrompt="",captions=true,effect="none",transition="cut"}=body||{};
    ...
    const payload={operation:"render_edit",workspaceId,projectId,mediaAssetId,requestedBy:user.id,startSeconds:start,endSeconds:end,title:String(title||asset.name||"Edited clip").slice(0,180),aspect:safeAspect,speed:safeSpeed,zoom:safeZoom,captions:Boolean(captions),effect:String(effect||"none"),transition:String(transition||"cut"),aiPrompt:String(aiPrompt||"").slice(0,2000)};
