...
export async function POST(request){
  try{
    ...
    const{workspaceId,projectId,mediaAssetId,startSeconds,endSeconds,title,aspect="9:16",speed=1,zoom=1,aiPrompt=""}=body||{};
    ...
    const payload={operation:"render_edit",workspaceId,projectId,mediaAssetId,requestedBy:user.id,startSeconds:start,endSeconds:end,title:String(title||asset.name||"Edited clip").slice(0,180),aspect:safeAspect,speed:safeSpeed,zoom:safeZoom,aiPrompt:String(aiPrompt||"").slice(0,2000)};
    const{data:job,error:jobError}=await admin.from("processing_jobs").insert({workspace_id:workspaceId,project_id:projectId,status:"queued",progress:1,payload}).select("id").single();
