...
export async function POST(request){
 try{
  ...
  if(!workerSecret)throw new Error("Media worker authentication is not configured.");
  const wr=await fetch(worker.replace(/\/$/,"")+"/process",{method:"POST",headers:{"content-type":"application/json","x-worker-secret":workerSecret},body:JSON.stringify({url:null,sourceType:"google_drive",workspaceId,projectId:project.data.id,sourceId:source.data.id,jobId:job.data.id,requestedBy:user.id,driveFileId:md.id,driveAccessToken:accessToken,driveFileName:md.name})});
