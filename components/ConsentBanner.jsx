...
export default function ConsentBanner({supabase}){
 ...
 if(!show)return null;
 return <div className="alphaConsentOverlay"><div className="alphaConsentCard"><div className="eyebrow">YOUR PRIVACY</div><h3>Before you continue</h3><p>Alpha.ai uses necessary cookies for sign-in and secure app operation. We record your Terms, Privacy and cookie decision with the policy version. We do not collect raw browser cookies or unrelated browsing activity.</p><div className="alphaConsentLinks"><a href="/legal#terms" target="_blank">Terms of Service</a><a href="/legal#privacy" target="_blank">Privacy Policy</a><a href="/legal#cookies" target="_blank">Cookie Policy</a></div><div className="alphaConsentActions"><button className="btn" disabled={busy} onClick={()=>save(false)}>Decline optional use</button><button className="btn primary" disabled={busy} onClick={()=>save(true)}>Agree & continue</button></div></div></div>;
}
